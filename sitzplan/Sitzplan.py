...
def main():
  ...
  with open(args.csvfile, newline='') as csvfile:
    ...
    height = max([int(p['row']) for p in people])
    width = max([int(p['column']) for p in people])
    ...
    for y in range(height):
      for x in range(width):
        ...
        # if not the first row, add the column separator
        if x > 0:
          matrix += "&"
    ...
    cmd = ['pdflatex', '-interaction', '-jobname ' + args.output, 'batchmode', 'temp.tex']
...
\tikzset{
  platz/.style={
    draw,
    text width=3cm,% <- gro§ genug wŠhlen
    align=center,
    minimum height=4\baselineskip% <- gro§ genug wŠhlen
}}
...
%      nodes in empty cells,
      ...
      %&&\sch{SchŸler 1}{2}{0}&\sch{SchŸler 2}{3}{1}&\sch{SchŸler 3}{2}{0}&\\
      %\sch{SchŸler 4}{3}{1}&\sch{SchŸler 5 A.}{2}{0}&\sch{SchŸler 8}{2}{-1}
      %  &\sch{SchŸler 9}{3}{1}&&\\
      %\sch{SchŸler 5 Z.}{2}{0}&\sch{SchŸler 6}{4}{2}&&\sch{SchŸler 7}{2}{-1}&&\\
  };
